...
class Handlers
{
	...
	*/
    ...
    //自定义参数设置
    public static var filterUrl_1 = "192.168.1.207:6006/api";
    public static var filterUrl_2 = "192.168.1.207:7006/api";
    public static var CommonPath = "E:\\02_产品相关\\01_QA_SVN项目\\新区大平台自动化测试系统\\System_Public_Lib";
    public static var filePath = CommonPath + "\\fiddler_sessions\\Input_TXT_Files\\";
    public static var filePathForRequested = CommonPath + "\\fiddler_sessions\\Requested.txt";
    public static var filePathForErrorResponse = CommonPath + "\\fiddler_sessions\\ErrorResponse.txt";
    public static var filePathForVerifyRequset = CommonPath + "\\fiddler_sessions\\VerifyRequset.txt";
    public static var filePathForRemoveSession = CommonPath + "\\fiddler_sessions\\RemoveSession.txt";
    public static var filePathForAddSession = CommonPath + "\\fiddler_sessions\\AddSession.txt";
	...
	function DoRemoveSession(oSessions: Session[]) {
		if (oSessions.Length > 1) {
			MessageBox.Show("Only select 1 session.");
			return;
        } else if (oSessions.Length == 0) {
        ...
        file.writeLine("----------------------------Session start--------------------------------" + "\n\n");
        file.writeLine("\n");
        file.writeLine("Request method: " + oSessions[0].RequestMethod);
        file.writeLine("\n");
		...
		s.Run("python replace_script_str\\Fiddler.py 3".replace("replace_script_str", CommonPath));
    }

	public static ContextAction("Add Session To Disk")
	function DoAddSession(oSessions: Session[]) {
        ...
        file.writeLine("----------------------------Session end--------------------------------" + "\r\n");
		file.close();
		//js 调用cmd执行python脚本进行添加
		var s = new ActiveXObject("WScript.Shell");
		s.Run("python replace_script_str\\Fiddler.py 2".replace("replace_script_str", CommonPath));
    }
        
    public static ContextAction("Generate RobotFramework Script")
    function DoGenerateRFSession(oSessions: Session[]) {
        //js 调用cmd执行python脚本进行转化
        var s = new ActiveXObject("WScript.Shell");
        s.Run("python replace_script_str\\Fiddler.py 1".replace("replace_script_str", CommonPath));
    }
	...
	*/
	...
	static function OnBeforeResponse(oSession: Session) {
        ...
        if (oSession.responseCode == 404 || oSession.responseCode == 500){
		...
		//过滤无关请求，只关注特定请求
		if (oSession.fullUrl.Contains(filterUrl_1) || oSession.fullUrl.Contains(filterUrl_2))
        {   
            var responseBody = oSession.GetResponseBodyAsString();
            var url = oSession.url;
            var Apiname = oSession.RequestMethod;
            ...
            {
                var reg = /^\d+$/;
                var result = url.Split("/")[i].match(reg);
                
                if (!result){
                    Apiname += "_" + url.Split("/")[i];
                    }
            }
             
            //MessageBox.Show(Apiname);
            //MessageBox.Show(filePath + Apiname + ".txt");
            ...
            if (oSession.responseCode != 200){ 
                ...
                file.writeLine("----------------------------Session start--------------------------------" + "\n\n");
                file.writeLine("Request method: " + oSession.RequestMethod);
                file.writeLine("\n");
                ...
                file.close();
                //oSession["ui-color"] = "red";
            } else if (responseBody.IndexOf("\"Success\":true") == 1){
                oSession.utilDecodeResponse();//消除保存的请求可能存在乱码的情况
                //保存服务器返回Success 不为true的请求
               ...
               // oSession["ui-color"] = "green";
                ...
                if (fso.FileExists(filePath + Apiname + ".txt")){
                    return;
                    }
                file = fso.OpenTextFile(filePath + Apiname + ".txt",8 ,true, true);
                file.writeLine("----------------------------Session start--------------------------------" + "\n\n");
                file.writeLine("File name: " + Apiname + ".txt");
                file.writeLine("\n");
                file.writeLine("Request method: " + oSession.RequestMethod);
                file.writeLine("\n");
                file.writeLine("Request url: " + url);
